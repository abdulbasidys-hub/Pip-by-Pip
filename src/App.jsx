...
const tradeMonthKey = d => (d||'').slice(0,7);
...
const buildMonthOptions = trades => {
  const keys = new Set(trades.map(t=>tradeMonthKey(t.date)).filter(k=>k.length===7));
  keys.add(currentMonthKey());
  return [...keys].sort().reverse();
};
const applyMonthFilter = (trades, month) =>
  month === 'all' ? trades : trades.filter(t => tradeMonthKey(t.date) === month);

// ─────────────────────────────────────────────────────────────
//  THEMES
// ─────────────────────────────────────────────────────────────
const DARK_THEME = `
  --bg:#0c0d12;--bg1:#11131a;--bg2:#161820;--bg3:#1c1e28;--bg4:#232633;
  --line:rgba(255,255,255,0.07);--line2:rgba(255,255,255,0.13);
  --accent:#e8a045;--accent-d:#c4832a;--accent-g:rgba(232,160,69,0.10);--accent-gs:rgba(232,160,69,0.05);
  --green:#4ade9a;--green-g:rgba(74,222,154,0.10);
  --red:#f06b6b;--red-g:rgba(240,107,107,0.10);
  --blue:#6db8ff;--text:#d4d7e8;--text2:#8b8fa8;--text3:#484c64;--white:#f0f2ff;`;
const LIGHT_THEME = `
  --bg:#f6f5f1;--bg1:#edecea;--bg2:#e7e6e2;--bg3:#dddbd8;--bg4:#d0ceca;
  --line:rgba(0,0,0,0.08);--line2:rgba(0,0,0,0.15);
  --accent:#b86a12;--accent-d:#9a5510;--accent-g:rgba(184,106,18,0.10);--accent-gs:rgba(184,106,18,0.06);
  --green:#16874e;--green-g:rgba(22,135,78,0.10);
  --red:#c43535;--red-g:rgba(196,53,53,0.10);
  --blue:#1f5fab;--text:#1e1d1a;--text2:#605e58;--text3:#9e9b94;--white:#0f0e0b;`;
...
:root{${DARK_THEME}
  --display:'DM Serif Display',serif;--sans:'Plus Jakarta Sans',sans-serif;--mono:'JetBrains Mono',monospace;
  --r:10px;--r2:14px;--sidebar-w:230px;}
[data-theme="light"]{${LIGHT_THEME}}
html,body{height:100%;background:var(--bg);color:var(--text);font-family:var(--sans);overflow-x:hidden;
  transition:background 0.3s,color 0.3s;font-size:15px;line-height:1.6;-webkit-font-smoothing:antialiased;}
...
.s0{animation-delay:0ms}.s1{animation-delay:60ms}.s2{animation-delay:120ms}
.s3{animation-delay:180ms}.s4{animation-delay:240ms}.s5{animation-delay:300ms}
...
.auth-card{background:var(--bg2);border:1px solid var(--line2);border-radius:18px;overflow:hidden;box-shadow:0 24px 80px rgba(0,0,0,0.25)}
...
.auth-desc{font-size:13px;color:var(--text2);line-height:1.75;margin-bottom:28px;font-weight:400}
.auth-err{background:var(--red-g);border:1px solid rgba(240,107,107,0.2);border-radius:var(--r);padding:11px 15px;font-size:12px;color:var(--red);margin-bottom:16px;font-weight:500}

/* FORM */
.f{margin-bottom:20px}
.f label{display:block;font-size:11px;letter-spacing:1.5px;text-transform:uppercase;color:var(--text2);margin-bottom:8px;font-weight:700}
.f input,.f select,.f textarea{width:100%;background:var(--bg3);border:1px solid var(--line2);border-radius:var(--r);
  padding:13px 15px;font-family:var(--sans);font-size:14px;color:var(--white);outline:none;
  transition:border-color 0.2s,box-shadow 0.2s,background 0.2s;-webkit-appearance:none;appearance:none;line-height:1.5;font-weight:500}
...
/* BUTTONS */
.btn{display:inline-flex;align-items:center;justify-content:center;gap:8px;padding:12px 22px;border-radius:var(--r);
  font-family:var(--sans);font-size:12px;font-weight:700;letter-spacing:1px;text-transform:uppercase;
...
.btn-full{width:100%}.btn-sm{padding:8px 16px;font-size:11px}
.btn:disabled{opacity:0.4;cursor:not-allowed;transform:none!important}

/* SHELL */
.shell{display:flex;min-height:100vh}
.sidebar{width:var(--sidebar-w);flex-shrink:0;background:var(--bg1);border-right:1px solid var(--line);
  display:flex;flex-direction:column;position:fixed;height:100vh;z-index:100;top:0;left:0;
...
.nb{width:100%;display:flex;align-items:center;gap:11px;padding:11px 14px;border-radius:var(--r);
  font-family:var(--sans);font-size:13px;font-weight:600;color:var(--text2);background:none;border:none;
  cursor:pointer;transition:all 0.18s;text-align:left;margin-bottom:3px;position:relative}
...
.user-tile{display:flex;align-items:center;gap:11px;padding:11px 13px;border-radius:var(--r);
  background:var(--bg2);border:1px solid var(--line);cursor:pointer;transition:all 0.18s;margin-bottom:10px}
.user-tile:hover{border-color:rgba(232,160,69,0.25);background:var(--bg3)}
.avatar{width:32px;height:32px;border-radius:8px;background:var(--accent-g);border:1px solid rgba(232,160,69,0.3);
  display:flex;align-items:center;justify-content:center;font-size:14px;color:var(--accent);flex-shrink:0;font-weight:800}
...
.so-btn{padding:7px 10px;background:none;border:none;font-family:var(--sans);font-size:11px;font-weight:600;
  color:var(--text3);cursor:pointer;transition:color 0.15s;border-radius:6px}
.so-btn:hover{color:var(--red);background:var(--red-g)}
.theme-btn{display:flex;align-items:center;gap:6px;padding:7px 12px;background:var(--bg2);border:1px solid var(--line2);
  border-radius:8px;font-family:var(--sans);font-size:11px;font-weight:600;color:var(--text2);cursor:pointer;transition:all 0.18s}
.theme-btn:hover{color:var(--accent);border-color:rgba(232,160,69,0.3)}

/* MOBILE */
.mobile-topbar{display:none;position:fixed;top:0;left:0;right:0;background:var(--bg1);
  border-bottom:1px solid var(--line);padding:13px 18px;z-index:200;align-items:center;justify-content:space-between}
...
.mobile-menu-btn{background:none;border:1px solid var(--line2);border-radius:8px;padding:8px 12px;cursor:pointer;
  color:var(--text);display:flex;align-items:center;gap:6px;font-size:12px;font-family:var(--sans);font-weight:700}
.mobile-overlay{display:none;position:fixed;inset:0;background:rgba(0,0,0,0.6);z-index:98}
.mobile-nav-bottom{display:none;position:fixed;bottom:0;left:0;right:0;background:var(--bg1);
  border-top:1px solid var(--line);padding:6px 0 calc(6px + env(safe-area-inset-bottom));z-index:200;
  align-items:center;justify-content:space-around}
.mnb{display:flex;flex-direction:column;align-items:center;gap:4px;padding:6px 12px;border:none;background:none;
  cursor:pointer;color:var(--text3);font-family:var(--sans);font-size:10px;font-weight:600;border-radius:8px;
...
.ph-ey{font-size:11px;letter-spacing:3px;text-transform:uppercase;color:var(--accent);margin-bottom:12px;
  display:flex;align-items:center;gap:10px;font-family:var(--mono);font-weight:500}
...
.ph-sub{font-size:14px;color:var(--text2);margin-top:10px;line-height:1.6;font-weight:400}

/* STATS */
.stat-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-bottom:24px}
.sc{background:var(--bg2);border:1px solid var(--line);border-radius:var(--r2);padding:22px 24px;
...
.sc-label{font-size:11px;letter-spacing:1px;text-transform:uppercase;color:var(--text2);margin-bottom:10px;font-weight:700}
...
.sc-val.wht{color:var(--white)}.sc-val.blu{color:var(--blue)}
.sc-sub{font-size:11px;color:var(--text3);margin-top:7px;font-weight:500}
...
.sc-stripe.green{background:var(--green)}.sc-stripe.red{background:var(--red)}.sc-stripe.blue{background:var(--blue)}

/* SECTION LABEL */
.sl{font-size:11px;letter-spacing:2px;text-transform:uppercase;color:var(--text2);margin-bottom:16px;
...
.curve-meta{font-size:12px;color:var(--text3);margin-top:5px;font-weight:500}

/* ASSET BARS */
.ab-list{display:flex;flex-direction:column;gap:6px;margin-bottom:24px}
.ab-row{display:grid;grid-template-columns:140px 1fr 72px 64px;align-items:center;gap:12px;
  background:var(--bg2);border:1px solid var(--line);border-radius:var(--r);padding:13px 18px;transition:all 0.15s}
.ab-row:hover{border-color:var(--line2);background:var(--bg3)}
.ab-name{font-size:13px;color:var(--text);font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
...
.tbl-wrap th{padding:13px 15px;font-size:11px;letter-spacing:1px;text-transform:uppercase;
  color:var(--text2);text-align:left;font-weight:700;white-space:nowrap}
...
.td-win{color:var(--green);font-weight:700}.td-loss{color:var(--red);font-weight:700}.td-be{color:var(--accent);font-weight:700}
.td-note{color:var(--text2);font-style:italic;max-width:180px;overflow:hidden;text-overflow:ellipsis}
.badge{display:inline-block;padding:4px 9px;border-radius:5px;font-size:10px;letter-spacing:0.5px;font-weight:700}
.bl{background:var(--green-g);color:var(--green)}.bb{background:rgba(109,184,255,0.1);color:var(--blue)}
.sess-badge{display:inline-block;padding:3px 8px;border-radius:4px;font-size:10px;font-weight:700;
  background:var(--accent-g);color:var(--accent)}
.empty-st{padding:64px;text-align:center;font-size:13px;letter-spacing:2px;color:var(--text3);font-weight:500}
.row-win{background:rgba(74,222,154,0.05)!important}.row-loss{background:rgba(240,107,107,0.05)!important}
.row-win:hover{background:rgba(74,222,154,0.10)!important}.row-loss:hover{background:rgba(240,107,107,0.10)!important}

/* TRADE DETAIL MODAL */
.trade-detail-overlay{position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:500;
  display:flex;align-items:center;justify-content:center;padding:24px;animation:pageIn 0.2s ease both}
.trade-detail-modal{background:var(--bg2);border:1px solid var(--line2);border-radius:18px;
  width:100%;max-width:620px;max-height:85vh;overflow-y:auto;box-shadow:0 32px 80px rgba(0,0,0,0.4)}
.tdm-header{padding:24px 28px 18px;border-bottom:1px solid var(--line);display:flex;
  justify-content:space-between;align-items:flex-start;position:sticky;top:0;background:var(--bg2);z-index:2}
...
.tdm-field-label{font-size:10px;letter-spacing:1.5px;text-transform:uppercase;color:var(--text3);margin-bottom:6px;font-weight:700}
.tdm-field-val{font-size:14px;color:var(--text);font-weight:600}
.tdm-notes{background:var(--bg3);border:1px solid var(--line);border-radius:10px;padding:14px 16px;margin-top:8px}
.tdm-notes-label{font-size:10px;letter-spacing:1.5px;text-transform:uppercase;color:var(--text3);margin-bottom:8px;font-weight:700}
.tdm-notes-val{font-size:13px;color:var(--text2);line-height:1.7}
...
.trade-card{background:var(--bg2);border:1px solid var(--line);border-radius:var(--r2);
...
.tc-asset{font-size:15px;font-weight:700;color:var(--white)}
...
.tc-note{font-size:13px;color:var(--text2);margin-top:8px;line-height:1.5}
...
.ring-lbl{font-size:10px;color:var(--text3);margin-top:3px;font-weight:600}

/* MONTH BAR */
.month-bar{display:flex;gap:6px;align-items:center;flex-wrap:wrap;margin-bottom:20px;
  padding:14px 18px;background:var(--bg2);border:1px solid var(--line);border-radius:var(--r2)}
.month-bar-label{font-size:10px;letter-spacing:2px;text-transform:uppercase;color:var(--text3);
  font-family:var(--mono);font-weight:600;margin-right:4px;flex-shrink:0}
.month-btn{padding:6px 14px;border-radius:20px;font-family:var(--sans);font-size:12px;font-weight:700;
  cursor:pointer;border:1px solid var(--line2);background:transparent;color:var(--text3);
  transition:all 0.15s;white-space:nowrap}
.month-btn:hover{border-color:rgba(232,160,69,0.3);color:var(--text)}
.month-btn.on{border-color:rgba(232,160,69,0.5);color:var(--accent);background:var(--accent-gs)}
.month-btn.current{border-color:rgba(232,160,69,0.2)}

/* FILTER TABS */
.ftabs{display:flex;gap:4px;background:var(--bg2);border:1px solid var(--line);border-radius:30px;padding:4px}
.ftab{padding:7px 16px;border-radius:24px;font-family:var(--sans);font-size:12px;font-weight:700;
  cursor:pointer;border:none;background:none;color:var(--text3);transition:all 0.18s}
...
.sort-label{font-size:11px;letter-spacing:1px;text-transform:uppercase;color:var(--text3);font-weight:700}
.sort-btn{padding:6px 14px;border-radius:20px;font-family:var(--sans);font-size:11px;font-weight:700;
...
.mpill{padding:9px 22px;border-radius:20px;font-family:var(--sans);font-size:13px;font-weight:700;
  cursor:pointer;border:1px solid var(--line2);background:var(--bg2);color:var(--text3);transition:all 0.18s}
...
.fp-title{font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:1px;color:var(--text2);
  margin-bottom:22px;padding-bottom:16px;border-bottom:1px solid var(--line);display:flex;align-items:center;gap:10px}
...
.commit-btn{width:100%;padding:16px;background:var(--accent);border:none;border-radius:var(--r);
  font-family:var(--sans);font-size:14px;font-weight:800;letter-spacing:1.5px;text-transform:uppercase;
  color:#fff;cursor:pointer;transition:all 0.2s;box-shadow:0 4px 20px rgba(0,0,0,0.15)}
...
/* STRATEGY STEPS */
.strat-step{background:var(--bg3);border:1px solid var(--line);border-radius:var(--r);
  padding:16px 18px;margin-bottom:8px;transition:border-color 0.18s,box-shadow 0.18s}
.strat-step:focus-within{border-color:rgba(232,160,69,0.3);box-shadow:0 0 0 2px var(--accent-g)}
.step-lbl{font-size:12px;font-weight:700;color:var(--text2);margin-bottom:10px;display:flex;align-items:center;gap:8px}
.step-num{width:18px;height:18px;border-radius:4px;background:var(--accent-g);border:1px solid rgba(232,160,69,0.2);
  display:flex;align-items:center;justify-content:center;font-size:11px;color:var(--accent);font-weight:800;font-family:var(--mono)}

/* CHECKBOX */
.ck-field{display:flex;align-items:center;gap:12px;cursor:pointer;padding:4px 0}
.ck-box{width:22px;height:22px;border-radius:6px;flex-shrink:0;border:1.5px solid var(--line2);
  background:var(--bg4);display:flex;align-items:center;justify-content:center;transition:all 0.18s}
.ck-box.on{background:var(--accent);border-color:var(--accent)}
.ck-lbl{font-size:14px;color:var(--text);transition:color 0.15s;font-weight:500}
.ck-field:hover .ck-lbl{color:var(--white)}

/* MULTISELECT */
.ms-wrap{display:flex;flex-wrap:wrap;gap:6px;margin-top:4px}
.ms-chip{padding:6px 12px;border-radius:20px;border:1px solid var(--line2);background:var(--bg3);
  color:var(--text3);font-size:12px;font-weight:600;cursor:pointer;transition:all 0.15s;user-select:none}
...
/* BUILDER */
.builder-shell{min-height:100vh;display:flex;align-items:flex-start;justify-content:center;
  position:relative;z-index:1;padding:60px 24px 80px;overflow-y:auto;background:var(--bg)}
.builder-wrap{width:100%;max-width:660px}
.b-step{font-size:11px;letter-spacing:2px;text-transform:uppercase;color:var(--accent);margin-bottom:12px;
  display:flex;align-items:center;gap:8px;font-family:var(--mono);font-weight:500}
...
.b-desc{font-size:14px;color:var(--text2);line-height:1.8;margin-bottom:34px}
.strat-name-area{margin-bottom:28px}
.strat-name-label{font-size:11px;letter-spacing:1.5px;text-transform:uppercase;color:var(--text2);margin-bottom:10px;font-weight:700}
.strat-name-input{font-family:var(--display);font-style:italic;font-size:30px;font-weight:600;
  background:transparent;border:none;border-bottom:1px solid var(--line2);border-radius:0;
  padding:6px 0;color:var(--white);width:100%;outline:none;transition:border-color 0.2s}
...
.fl-row{display:flex;align-items:center;justify-content:space-between;background:var(--bg2);
  border:1px solid var(--line);border-radius:var(--r);padding:12px 16px;transition:all 0.15s}
.fl-row:hover{border-color:var(--line2)}
.fl-left{display:flex;align-items:center;gap:12px}
.fl-num{width:24px;height:24px;border-radius:6px;background:var(--accent-g);border:1px solid rgba(232,160,69,0.2);
...
.fl-name{font-size:14px;color:var(--text);font-weight:600}
.fl-type{font-size:11px;color:var(--text3);margin-top:3px;font-weight:500}
.fl-acts{display:flex;gap:4px}
.ib{background:none;border:none;cursor:pointer;color:var(--text3);padding:5px;
  display:flex;align-items:center;transition:color 0.12s;border-radius:4px}
.ib:hover{color:var(--text);background:var(--bg3)}
.ib.del:hover{color:var(--red)}
.ib.edit-btn:hover{color:var(--accent)}
.add-trig{width:100%;padding:16px;border:1.5px dashed var(--line2);border-radius:var(--r);
  background:none;font-family:var(--sans);font-size:9px;letter-spacing:3px;text-transform:uppercase;
  color:var(--text3);cursor:pointer;font-weight:700;transition:all 0.18s;margin-bottom:16px;
  display:flex;align-items:center;justify-content:center;gap:8px}
.add-trig:hover{border-color:rgba(232,160,69,0.4);color:var(--accent);background:var(--accent-gs)}
.add-panel{background:var(--bg2);border:1px solid var(--line2);border-radius:var(--r2);
  padding:22px;margin-bottom:14px;animation:pageIn 0.25s cubic-bezier(0.16,1,0.3,1)}
.add-panel-title{font-size:12px;font-weight:700;color:var(--text2);margin-bottom:16px;text-transform:uppercase;letter-spacing:1px}
.type-tabs{display:flex;gap:6px;margin-bottom:16px;flex-wrap:wrap}
.type-tab{padding:7px 14px;border-radius:6px;font-family:var(--sans);font-size:12px;font-weight:700;
  cursor:pointer;border:1px solid var(--line2);background:var(--bg3);color:var(--text3);transition:all 0.15s}
...
.ot{display:inline-flex;align-items:center;gap:5px;padding:5px 12px;background:var(--bg3);
  border:1px solid var(--line2);border-radius:5px;font-size:12px;color:var(--text2);font-weight:500}
.ot button{background:none;border:none;color:var(--text3);cursor:pointer;font-size:13px;line-height:1;padding:0}
...
.ins-label{font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:1px;color:var(--text2);margin-bottom:10px}
.ins-val{font-family:var(--display);font-size:34px;line-height:1.1;font-style:italic;font-weight:600}
.ins-sub{font-size:12px;color:var(--text3);margin-top:5px;font-weight:500}
.ai-panel{background:var(--bg2);border:1px solid var(--line);border-radius:var(--r2);
...
.ai-icon{width:32px;height:32px;border-radius:8px;background:var(--accent-g);
  border:1px solid rgba(232,160,69,0.3);display:flex;align-items:center;justify-content:center;font-size:16px}
.ai-title{font-size:14px;font-weight:800;color:var(--white)}
.ai-sub{font-size:12px;color:var(--text3);font-weight:500}
.ai-insights{display:flex;flex-direction:column;gap:10px}
.ai-item{display:flex;align-items:flex-start;gap:12px;padding:14px 16px;background:var(--bg3);
  border:1px solid var(--line);border-radius:var(--r)}
.ai-dot{width:8px;height:8px;border-radius:50%;flex-shrink:0;margin-top:4px}
.ai-item-title{font-size:13px;font-weight:700;color:var(--white);margin-bottom:5px}
.ai-item-body{font-size:13px;color:var(--text2);line-height:1.6}

/* FILTER PANEL */
.filter-bar{background:var(--bg2);border:1px solid var(--line);border-radius:var(--r2);padding:20px 24px;margin-bottom:24px}
.filter-title{font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:1px;color:var(--text2);
...
.flt-group label{display:block;font-size:11px;letter-spacing:1px;text-transform:uppercase;
  color:var(--text2);margin-bottom:7px;font-weight:700}
.flt-group select,.flt-group input{width:100%;background:var(--bg3);border:1px solid var(--line2);
  border-radius:8px;padding:10px 13px;font-family:var(--sans);font-size:13px;color:var(--text);
  font-weight:500;outline:none;-webkit-appearance:none;appearance:none;transition:border-color 0.2s}
...
.flt-count{font-size:12px;color:var(--text3);font-weight:500}
.export-area{display:flex;gap:8px;align-items:center;margin-bottom:20px;flex-wrap:wrap}
.export-btn{display:flex;align-items:center;gap:6px;padding:8px 16px;border-radius:8px;
  border:1px solid var(--line2);background:var(--bg2);color:var(--text2);font-family:var(--sans);
  font-size:11px;font-weight:700;letter-spacing:0.5px;cursor:pointer;transition:all 0.18s}
.export-btn:hover{border-color:rgba(232,160,69,0.4);color:var(--accent);background:var(--accent-gs)}

/* SETTINGS */
.settings-wrap{max-width:660px}
.saved-flash{font-size:12px;color:var(--green);letter-spacing:2px;text-transform:uppercase;font-weight:700}
.div{width:100%;height:1px;background:var(--line);margin:24px 0}

/* LOADER */
.loader-sh{min-height:100vh;display:flex;align-items:center;justify-content:center;
  background:var(--bg);flex-direction:column;gap:16px}
.loader-ring{width:36px;height:36px;border:2px solid var(--line2);border-top-color:var(--accent);
  border-radius:50%;animation:spin 0.8s linear infinite}
.loader-txt{font-size:12px;letter-spacing:3px;text-transform:uppercase;color:var(--text3);font-weight:600}
...
@media(max-width:768px){
  ...
  .mobile-topbar{display:flex}.mobile-overlay.open{display:block}.mobile-nav-bottom{display:flex}
  .main{margin-left:0;padding-top:60px;padding-bottom:70px}
  .page{padding:22px 18px 32px}.ph{margin-bottom:26px}.ph-title{font-size:40px}
  .stat-grid{grid-template-columns:1fr 1fr;gap:8px}
  .sc{padding:16px 18px}.sc-val{font-size:28px}.sc-label{font-size:10px}.sc-sub{font-size:10px}
  .ins-grid{grid-template-columns:1fr}.fg2,.fg3,.fg4{grid-template-columns:1fr 1fr}
  .tbl-wrap table{display:none}.trade-cards{display:flex}
  .ab-row{grid-template-columns:100px 1fr 60px 50px;padding:10px 12px;gap:8px}
  .curve-card{padding:18px 20px}.auth-card-body{padding:28px 24px}.brand-mark{font-size:52px}
  .filter-grid{grid-template-columns:1fr 1fr}.export-area{gap:6px}.ai-panel{padding:18px 20px}
  .tdm-grid{grid-template-columns:1fr}
  .month-bar{padding:10px 14px;gap:5px}.month-btn{padding:5px 10px;font-size:11px}
}
@media(max-width:480px){
  .fg2,.fg3,.fg4{grid-template-columns:1fr}.filter-grid{grid-template-columns:1fr}
  .stat-grid{grid-template-columns:1fr 1fr}.ftabs{display:none}
}`;
...
const fmtD = d => {
  if(!d) return '—';
  try { return new Date(d+'T12:00:00').toLocaleDateString('en-GB',{day:'2-digit',month:'short',year:'2-digit'}); }
  catch { return d; }
};
...
function AnimNum({ val, dec=1, prefix='', suffix='' }) {
  ...
  useEffect(() => {
    ...
    const tick = ts => {
      ...
      setDisp(target*(1-Math.pow(1-p,4)));
...
// ─── MonthBar ─────────────────────────────────────────────────────────────────
function MonthBar({ trades, month, setMonth }) {
  const cur = currentMonthKey();
  const months = useMemo(() => buildMonthOptions(trades), [trades]);
  return (
    <div className="month-bar">
      <span className="month-bar-label">Month</span>
      <button className={`month-btn ${month==='all'?'on':''}`} onClick={()=>setMonth('all')}>All time</button>
      {months.map(m => (
        <button key={m} className={`month-btn ${month===m?'on':''} ${m===cur?'current':''}`} onClick={()=>setMonth(m)}>
          {fmtMonthLabel(m)}{m===cur?' ●':''}
        </button>
      ))}
    </div>
  );
    ...
    <div className="curve-card page-enter s1">
      <div className="curve-top">
        <div>
          ...
          <div className="curve-r" style={{color:col}}><AnimNum val={last} dec={2} prefix={last>=0?'+':''} suffix="R"/></div>
      ...
      <svg width="100%" viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none" style={{height:110,display:'block'}}>
        ...
        <path d={path} fill="none" stroke={col} strokeWidth="2"
          ...
          style={{animation:'drawLine 1.5s 0.3s cubic-bezier(0.16,1,0.3,1) forwards',['--len']:'2000'}}/>
...
// ─── Export CSV ───────────────────────────────────────────────────────────────
function exportCSV(trades, label='trades', fields=[]) {
  const headers = ['Date','Mode','Asset','Session','Outcome','Result (R)','Notes',...fields.map(f=>f.name)];
  const lines = [headers.join(',')];
  trades.forEach(t => {
    const sv = fields.map(f => {
    ...
    lines.push([
      t.date||'', t.mode||'', `"${(t.asset||'').replace(/"/g,'""')}"`,
      t.session||'No Session', t.outcome||'', t.resultR||0,
      `"${(t.notes||'').replace(/"/g,'""')}"`, ...sv
    ].join(','));
  });
  const a = document.createElement('a');
  a.href = URL.createObjectURL(new Blob([lines.join('\n')],{type:'text/csv'}));
  a.download = `pip-by-pip-${label}-${new Date().toISOString().split('T')[0]}.csv`;
  a.click();
}
...
function buildSmartInsights(trades) {
  ...
  const ins=[], wins=closed.filter(t=>t.outcome==='Win'), wr=wins.length/closed.length;
  const netR=closed.reduce((s,t)=>s+parseFloat(t.resultR||0),0);
  if(wr>=0.6) ins.push({type:'strength',title:`Strong win rate at ${(wr*100).toFixed(0)}%`,body:'You are winning more than 6 out of 10 trades — that is consistency. Keep executing your rules.'});
  else if(wr<=0.4) ins.push({type:'weakness',title:`Win rate is ${(wr*100).toFixed(0)}% — focus on quality`,body:'Fewer than 4 in 10 trades close as wins. Consider waiting for higher-conviction setups only.'});
  else ins.push({type:'neutral',title:`Win rate at ${(wr*100).toFixed(0)}%`,body:'Track whether you are holding winners long enough — average R matters more than win rate alone.'});
  const am={};
  closed.forEach(t=>{if(!t.asset)return;if(!am[t.asset])am[t.asset]={r:0,n:0,w:0};am[t.asset].r+=parseFloat(t.resultR||0);am[t.asset].n++;if(t.outcome==='Win')am[t.asset].w++;});
  const byR=Object.entries(am).sort((a,b)=>b[1].r-a[1].r);
  if(byR.length>0){const[bA,b]=byR[0],[wA,w]=byR[byR.length-1];ins.push({type:'strength',title:`${bA} is your strongest pair`,body:`Net ${rSign(b.r)} across ${b.n} trades — ${b.n?((b.w/b.n)*100).toFixed(0):0}% WR. This is where your edge lives.`});if(byR.length>1&&w.r<0)ins.push({type:'weakness',title:`${wA} is costing you pips`,body:`Net ${rSign(w.r)} on this pair. Consider pausing it until you understand the pattern.`});}
  const sm={};
  closed.forEach(t=>{const s=t.session||'No Session';if(!sm[s])sm[s]={r:0,n:0,w:0};sm[s].r+=parseFloat(t.resultR||0);sm[s].n++;if(t.outcome==='Win')sm[s].w++;});
  const hasSess=Object.keys(sm).some(k=>k!=='No Session'&&sm[k].n>0);
  if(hasSess){const bySess=Object.entries(sm).filter(([k])=>k!=='No Session').sort((a,b)=>b[1].r-a[1].r);if(bySess.length>0){const[bS,bs]=bySess[0];ins.push({type:'strength',title:`${bS} session is your sweet spot`,body:`Net ${rSign(bs.r)} across ${bs.n} trades in this window.`});if(bySess.length>1){const[wS,ws]=bySess[bySess.length-1];if(ws.r<0)ins.push({type:'weakness',title:`${wS} session is draining your account`,body:`Net ${rSign(ws.r)} here. You may be overtrading or ignoring session conditions.`});}}}
  const dm={};
  closed.forEach(t=>{if(!t.date)return;const d=['Sun','Mon','Tue','Wed','Thu','Fri','Sat'][new Date(t.date+'T12:00:00').getDay()];if(!dm[d])dm[d]={r:0,n:0};dm[d].r+=parseFloat(t.resultR||0);dm[d].n++;});
  const wdays=['Mon','Tue','Wed','Thu','Fri'].filter(d=>dm[d]);
  if(wdays.length>2){const sd=[...wdays].sort((a,b)=>dm[b].r-dm[a].r);const worst=sd[sd.length-1];if(dm[worst].r<0&&worst!==sd[0])ins.push({type:'neutral',title:`Worst day: ${worst}`,body:`Net ${rSign(dm[worst].r)} on ${worst}s. Consider being more selective that day.`});}
  if(netR>0) ins.push({type:'strength',title:'You are net profitable — keep the discipline',body:`Total net R is ${rSign(netR)} across ${closed.length} trades. Protect your capital and let the edge compound.`});
  else ins.push({type:'weakness',title:'Still in the red — stay patient',body:`Net R is ${rSign(netR)}. Focus on risk first: make sure every loss is exactly 1R, never more.`});
  return ins;
}
    ...
    <div className="trade-detail-overlay" onClick={e=>{if(e.target===e.currentTarget)onClose()}}>
      <div className="trade-detail-modal">
        ...
        <div className="tdm-footer">
          <button className="btn btn-ghost btn-sm" onClick={()=>onEdit(trade)} style={{display:'flex',alignItems:'center',gap:6}}>{I.Edit} Edit Trade</button>
          <button className="btn btn-danger btn-sm" onClick={()=>onDelete(trade.id)} style={{display:'flex',alignItems:'center',gap:6}}>{I.Trash} Delete</button>
...
export default function App() {
  ...
  useEffect(()=>{ document.documentElement.setAttribute('data-theme',theme); localStorage.setItem('pbp-theme',theme); },[theme]);

  useEffect(()=>{
    let m=true;
    (async()=>{
      try{ if(typeof __initial_auth_token!=='undefined'&&__initial_auth_token){const{signInWithCustomToken:sct}=await import('firebase/auth');await sct(auth,__initial_auth_token);} }catch{}
    })();
    return onAuthStateChanged(auth,u=>{ if(m)setAuthUser(u); });
  },[]);

  useEffect(()=>{ if(!authUser){setProfile(null);return;} getDoc(userDoc(authUser.uid)).then(s=>{ setProfile(s.exists()?s.data():null); }); },[authUser]);
  ...
  const addTrade = async t => { await setDoc(tradeDoc(genId()),{...t,userId:authUser.uid,createdAt:Date.now()}); setView('journal'); };
  const updateTrade = async (id,t) => { await setDoc(tradeDoc(id),{...t,userId:authUser.uid},{merge:true}); setEditingTrade(null); setView('journal'); };
  ...
  if(authUser===undefined) return (<><style>{S}</style><div className="loader-sh"><div className="loader-ring"/><div className="loader-txt">Loading</div></div></>);
  if(!authUser||!profile) return (<><style>{S}</style><AuthFlow onComplete={p=>setProfile(p)}/></>);
    ...
    <>
      ...
      <div className="shell">
        <nav className="sidebar">
          <div className="sb-top"><div className="sb-brand"><div className="sb-logo">Pip <span>by</span> Pip</div><div className="sb-ver">Trade Journal</div></div></div>
        </nav>
        <main className="main">
          <EditTradePage profile={profile} trade={editingTrade} onSave={(id,t)=>updateTrade(id,t)} onCancel={()=>setEditingTrade(null)}/>
        </main>
      </div>
    </>
  );
    ...
    <>
      ...
      <div className="mobile-topbar">
        ...
        <div style={{display:'flex',gap:8,alignItems:'center'}}>
          ...
          <button className="mobile-menu-btn" onClick={()=>setMenuOpen(o=>!o)}>{menuOpen?I.X:I.Menu}</button>
        </div>
      </div>
        ...
        <nav className={`sidebar ${menuOpen?'open':''}`}>
          ...
          <div className="sb-bottom">
            ...
            <div className="sb-actions">
              <button className="theme-btn" onClick={()=>setTheme(t=>t==='dark'?'light':'dark')}>{theme==='dark'?<>{I.Sun} Light</>:<>{I.Moon} Dark</>}</button>
...
function AuthFlow({onComplete}) {
  ...
  const tl={dropdown:'Dropdown',text:'Free Text',checkbox:'Tick',multiselect:'Multi-Select'};

  const doLogin=async e=>{
    e.preventDefault();setErr('');setLoading(true);
    try{const u=await signInWithEmailAndPassword(auth,toEmail(form.username),form.password);const snap=await getDoc(userDoc(u.user.uid));if(snap.exists())onComplete(snap.data());else setErr('Account not found.');}
    catch{setErr('Invalid username or password.');}finally{setLoading(false);}
  };
  const doRegister=async e=>{
    ...
    setLoading(true);
    try{const u=await createUserWithEmailAndPassword(auth,toEmail(form.username),form.password);setNewUser(u.user);setStep('build');}
    catch(e){if(e.code==='auth/email-already-in-use')setErr('Username taken. Try another.');else setErr('Could not create account. Try again.');}
  ...
  const doFinish=async()=>{
    ...
    await setDoc(userDoc(newUser.uid),p);onComplete(p);setLoading(false);
  };
      ...
      <div className="auth-wrap">
        ...
        <div className="auth-card">
          <div className="auth-card-body">
            ...
            <div className="auth-desc">{step==='register'?'Pick a username and password. No email needed — your username is your identity.':'Enter your username and password to access your journal.'}</div>
            {err&&<div className="auth-err">{err}</div>}
            <form onSubmit={step==='register'?doRegister:doLogin}>
              <div className="f"><label>Username</label><input type="text" required value={form.username} autoComplete="username" onChange={e=>setForm({...form,username:e.target.value})} placeholder="e.g. sadik_trader"/></div>
              <div className="f"><label>Password</label><input type="password" required value={form.password} autoComplete={step==='register'?'new-password':'current-password'} onChange={e=>setForm({...form,password:e.target.value})} placeholder="••••••••"/></div>
              {step==='register'&&<div className="f"><label>Confirm Password</label><input type="password" required value={form.confirm} autoComplete="new-password" onChange={e=>setForm({...form,confirm:e.target.value})} placeholder="••••••••"/></div>}
      ...
      <div className="builder-wrap">
          ...
          <div className="fl-list">
            {fields.map((f,i)=>(
              <div key={f.id} className="fl-row">
                <div className="fl-left"><div className="fl-num">{i+1}</div><div><div className="fl-name">{f.name}</div><div className="fl-type">{tl[f.type]}{(f.type==='dropdown'||f.type==='multiselect')&&f.options.length?` · ${f.options.join(', ')}`:''}</div></div></div>
                <div className="fl-acts"><button className="ib" onClick={()=>mvField(i,-1)}>{I.Up}</button><button className="ib" onClick={()=>mvField(i,1)}>{I.Down}</button><button className="ib del" onClick={()=>rmField(f.id)}>{I.Trash}</button></div>
              </div>
            ))}
          </div>
        )}
        {adding?(
          <div className="add-panel">
            ...
            <div className="f"><label>Name / Question</label><input type="text" value={nf.name} autoFocus onChange={e=>setNf({...nf,name:e.target.value})} placeholder="e.g. HTF Bias? / Clear BOS? / Liquidity Target…"/></div>
            <div style={{marginBottom:16}}>
              ...
              <div className="type-tabs">{['dropdown','multiselect','text','checkbox'].map(t=>(<button key={t} className={`type-tab ${nf.type===t?'on':''}`} onClick={()=>setNf({...nf,type:t})}>{tl[t]}</button>))}</div>
            </div>
            {(nf.type==='dropdown'||nf.type==='multiselect')&&(
              <div className="f"><label>Options</label>
                <div className="opts-row"><div className="f" style={{margin:0,flex:1}}><input type="text" value={nf.optIn} placeholder="Add option…" onChange={e=>setNf({...nf,optIn:e.target.value})} onKeyDown={e=>{if(e.key==='Enter'){e.preventDefault();if(nf.optIn.trim())setNf({...nf,options:[...nf.options,nf.optIn.trim()],optIn:''});}}}/></div><button className="btn btn-ghost btn-sm" onClick={()=>{if(nf.optIn.trim())setNf({...nf,options:[...nf.options,nf.optIn.trim()],optIn:''});}}>Add</button></div>
                {!!nf.options.length&&<div className="opt-tags">{nf.options.map((o,i)=>(<span key={i} className="ot">{o}<button onClick={()=>setNf({...nf,options:nf.options.filter((_,j)=>j!==i)})}>×</button></span>))}</div>}
              </div>
            )}
            <div style={{display:'flex',gap:8}}><button className="btn btn-primary" onClick={addField}>Save Step</button><button className="btn btn-ghost" onClick={()=>{setAdding(false);setNf({name:'',type:'dropdown',options:[],optIn:''});}}>Cancel</button></div>
          </div>
        ):(
          <button className="add-trig" onClick={()=>setAdding(true)}>{I.Plus} Add a strategy step</button>
        )}
        <button className="commit-btn" onClick={doFinish} disabled={loading}>{loading?'Saving…':"I'm done — Open my journal →"}</button>
...
function DashboardPage({trades,profile}) {
  ...
  const filtered = useMemo(()=>{
    let t = scope==='all'?trades:trades.filter(x=>x.mode===scope);
    return applyMonthFilter(t,month);
  },[trades,scope,month]);
  ...
  const stats = useMemo(()=>{
    ...
    let streak=0;
    [...v].sort((a,b)=>new Date(b.date)-new Date(a.date)).forEach(t=>{if(t.outcome==='Win')streak++;else if(streak===0){}});
    // recount properly
    streak=0;
    for(const t of [...v].sort((a,b)=>new Date(b.date)-new Date(a.date)||((b.createdAt||0)-(a.createdAt||0)))){if(t.outcome==='Win')streak++;else break;}
    return {wr,r,total:filtered.length,valid:v.length,wins:w.length,avg,streak};
  },[filtered]);

  // Most recent 8, newest first (descending)
  const recent = useMemo(()=>
    [...filtered].sort((a,b)=>(new Date(b.date)-new Date(a.date))||((b.createdAt||0)-(a.createdAt||0))).slice(0,8)
  ,[filtered]);

  const fields=profile.strategy?.fields||[];
  const monthLabel=month==='all'?'All time':fmtMonthLabel(month);
  ...
  return (
    <div className="page page-enter">
      <div className="ph">
        ...
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-end',flexWrap:'wrap',gap:16}}>
          <div>
            ...
            <div className="ph-sub">{profile.strategy?.name||'No strategy set'} · {stats.total} trades · {monthLabel}</div>
          </div>
          <div className="ftabs">
      ...
      <MonthBar trades={trades} month={month} setMonth={setMonth}/>
      ...
      <div className="tbl-wrap page-enter s3">
        {!recent.length?<div className="empty-st">No trades for this period.</div>:(
          <>
                ...
                <tbody>
                  {recent.map((t,idx)=>(
                    <tr key={t.id} className={rowClass(t)}>
                      ...
                      {fields.slice(0,2).map(f=>{const v=t.strategyData?.[f.id];const d=f.type==='checkbox'?(v?'✓':'—'):Array.isArray(v)?v.join(', '):(v||'—');return <td key={f.id} style={{color:'var(--text2)',maxWidth:110,overflow:'hidden',textOverflow:'ellipsis'}}>{d}</td>;})}
...
function JournalPage({trades,fields,onDelete,onEdit}) {
  const [scope,setScope] = useState('all');
  const [month,setMonth] = useState(currentMonthKey);
  // Default: date descending — newest on top
  ...
  const sorted = useMemo(()=>{
    ...
    arr = applyMonthFilter(arr,month);
    return [...arr].sort((a,b)=>{
      if(sortBy==='date') {
        const dd = new Date(a.date||0)-new Date(b.date||0);
        if(dd!==0) return sortDir==='asc'?dd:-dd;
        // Same date: use createdAt to keep add-order stable
        const cd = (a.createdAt||0)-(b.createdAt||0);
        return sortDir==='asc'?cd:-cd;
      }
      let va,vb;
      if(sortBy==='r'){va=parseFloat(a.resultR||0);vb=parseFloat(b.resultR||0);}
      else if(sortBy==='asset'){va=a.asset||'';vb=b.asset||'';}
      else if(sortBy==='outcome'){va=a.outcome||'';vb=b.outcome||'';}
      // Tiebreak: always date desc then createdAt desc so within-group newest first
      if(va===vb) return (new Date(b.date||0)-new Date(a.date||0))||((b.createdAt||0)-(a.createdAt||0));
      if(va<vb) return sortDir==='asc'?-1:1;
      if(va>vb) return sortDir==='asc'?1:-1;
      return 0;
    });
  },[trades,scope,month,sortBy,sortDir]);
  ...
  const handleDelete = async id => { await onDelete(id); setSelectedTrade(null); setConf(null); };
  const monthLabel = month==='all'?'All time':fmtMonthLabel(month);

  return (
    <div className="page page-enter">
      {selectedTrade&&(
        <TradeDetailModal trade={selectedTrade} fields={fields} onClose={()=>setSelectedTrade(null)} onEdit={t=>{onEdit(t);setSelectedTrade(null);}} onDelete={handleDelete}/>
      )}
      <div className="ph">
        <div className="ph-ey">Journal</div>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-end',flexWrap:'wrap',gap:16}}>
          <div><div className="ph-title">All Trades</div><div className="ph-sub">{sorted.length} entries · {monthLabel}</div></div>
          <div style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap'}}>
            ...
            <button className="export-btn" onClick={()=>exportCSV(sorted,`journal-${monthLabel}`,fields)}>{I.Download} Export CSV</button>
          </div>
        </div>
      </div>
    ...
    <div className="page page-enter">
      ...
      <form onSubmit={submit} style={{maxWidth:820}}>
        <div className="fp page-enter s0">
          ...
          <div style={{marginTop:8,fontSize:12,color:'var(--text3)',fontFamily:'var(--mono)'}}>Loss values are automatically stored as negative.</div>
        </div>
        {!!fields.length&&(
        ...
        <div className="fp page-enter s2">
          ...
          <div className="f" style={{margin:0}}><textarea value={base.notes} onChange={e=>setBase({...base,notes:e.target.value})} placeholder="What went well? What would you change?"/></div>
        </div>
        <div style={{display:'flex',gap:12}}>
          <button type="submit" className="commit-btn" disabled={saving} style={{flex:1}}>{saving?'Saving…':'Save Changes →'}</button>
          <button type="button" className="btn btn-ghost" onClick={onCancel} style={{padding:'16px 24px',fontSize:13}}>Cancel</button>
        </div>
      </form>
    </div>
  );
  ...
  return (
    <div className="page page-enter">
      ...
      <form onSubmit={submit} style={{maxWidth:820}}>
        ...
        <div className="fp page-enter s2">
          <div className="fp-title">Post-Trade Notes</div>
          <div className="f" style={{margin:0}}><textarea value={base.notes} onChange={e=>setBase({...base,notes:e.target.value})} placeholder="What went well? What would you change? Any market observations…"/></div>
        </div>
        <button type="submit" className="commit-btn" disabled={saving}>{saving?'Committing…':'Commit to Journal →'}</button>
      </form>
    </div>
  );
}
...
function InsightsPage({trades,fields}) {
  ...
  const stats = useMemo(()=>{
    const v=filtered.filter(t=>t.outcome!=='No Trade');
    const w=v.filter(t=>t.outcome==='Win'), l=v.filter(t=>t.outcome==='Loss');
    ...
    [...v].sort((a,b)=>new Date(a.date)-new Date(b.date)).forEach(t=>{cum+=parseFloat(t.resultR||0);if(cum>peak)peak=cum;if(peak-cum>dd)dd=peak-cum;});
    const days={};
    v.forEach(t=>{const d=['Sun','Mon','Tue','Wed','Thu','Fri','Sat'][new Date(t.date+'T12:00:00').getDay()];if(!days[d])days[d]={r:0,n:0};days[d].r+=parseFloat(t.resultR||0);days[d].n++;});
    const sessions={};
    v.forEach(t=>{const s=t.session||'No Session';if(!sessions[s])sessions[s]={r:0,n:0,w:0};sessions[s].r+=parseFloat(t.resultR||0);sessions[s].n++;if(t.outcome==='Win')sessions[s].w++;});
  ...
  return (
    <div className="page page-enter">
      <div className="ph">
        ...
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-end',flexWrap:'wrap',gap:16}}>
          <div><div className="ph-title">Numbers<br/><em>Don't Lie</em></div><div className="ph-sub">{monthLabel}</div></div>
          <div className="ftabs">
            {[['all','All'],['Live','Live'],['Backtest','Backtest']].map(([v,l])=>(
              <button key={v} className={`ftab ${scope===v?'on':''}`} onClick={()=>setScope(v)}>{l}</button>
            ))}
      ...
      <div className="export-area">
        <button className="export-btn" onClick={()=>exportCSV(filtered,`insights-${monthLabel}`,fields)}>{I.Download} Export View</button>
        <button className="export-btn" onClick={()=>exportCSV(trades,'all',fields)}>{I.Download} All Trades</button>
        <button className="export-btn" onClick={()=>setShowFilters(f=>!f)} style={hasActiveFilter?{borderColor:'rgba(232,160,69,0.5)',color:'var(--accent)'}:{}}>{I.Filter} {showFilters?'Hide Filters':'Filter'}{hasActiveFilter&&' •'}</button>
      </div>
      ...
      {showFilters&&(
        <div className="filter-bar page-enter">
          ...
          <div className="filter-grid">
            <div className="flt-group"><label>Asset</label>
              <select value={filters.asset} onChange={e=>setFilters({...filters,asset:e.target.value})}>
                <option value="">All pairs</option>{allAssets.map(a=><option key={a} value={a}>{a}</option>)}
              </select>
            </div>
            ...
            <div className="flt-group"><label>Outcome</label>
              <select value={filters.outcome} onChange={e=>setFilters({...filters,outcome:e.target.value})}>
                <option value="">All outcomes</option>{['Win','Loss','Breakeven','No Trade'].map(o=><option key={o} value={o}>{o}</option>)}
              </select>
            </div>
            <div className="flt-group"><label>Date From</label><input type="date" value={filters.dateFrom} onChange={e=>setFilters({...filters,dateFrom:e.target.value})}/></div>
            <div className="flt-group"><label>Date To</label><input type="date" value={filters.dateTo} onChange={e=>setFilters({...filters,dateTo:e.target.value})}/></div>
          </div>
          <div className="filter-actions">
            <span className="flt-count">{filtered.length} trade{filtered.length!==1?'s':''} match</span>
            {hasActiveFilter&&<button className="btn btn-ghost btn-sm" onClick={()=>setFilters({asset:'',session:'',outcome:'',dateFrom:'',dateTo:''})}>Clear</button>}
          </div>
        </div>
      )}
      ...
      <div className="ai-panel page-enter s1">
        <div className="ai-header">
          ...
          <div>
            ...
            <div className="ai-sub">Powered by your own trade data · {filtered.filter(t=>t.outcome!=='No Trade').length} trades analysed · {monthLabel}</div>
          </div>
        </div>
      ...
      {showFilters&&filtered.length>0&&(
        <>
          ...
          <div className="tbl-wrap">
            <div className="tbl-scroll">
              <table>
                <thead><tr><th>Date</th><th>Mode</th><th>Asset</th><th>Session</th><th>Outcome</th><th>R</th><th>Notes</th></tr></thead>
                <tbody>
                  {[...filtered].sort((a,b)=>(new Date(b.date)-new Date(a.date))||((b.createdAt||0)-(a.createdAt||0))).map(t=>(
                    <tr key={t.id} className={rowClass(t)}>
                      <td className="td-date">{fmtD(t.date)}</td>
                      <td><span className={`badge ${t.mode==='Live'?'bl':'bb'}`}>{t.mode}</span></td>
                      <td style={{color:'var(--text)',fontWeight:600}}>{t.asset||'—'}</td>
...
function SettingsPage({profile,onSave}) {
  ...
  const startEdit = f => { setEditingField(f.id); setNf({name:f.name,type:f.type,options:f.options||[],optIn:''}); setAdding(true); };
  const cancelEdit = () => { setEditingField(null); setAdding(false); setNf({name:'',type:'dropdown',options:[],optIn:''}); };
      ...
      <div className="settings-wrap">
          ...
          {strat.fields.map((f,i)=>(
            <div key={f.id} className="fl-row page-enter">
              <div className="fl-left">
                ...
                <div><div className="fl-name">{f.name}</div><div className="fl-type">{tl[f.type]||f.type}{(f.type==='dropdown'||f.type==='multiselect')&&f.options?.length?` · ${f.options.join(', ')}`:''}</div></div>
              </div>
        ...
        {adding?(
          <div className="add-panel">
            ...
            <div className="f"><label>Name</label><input type="text" value={nf.name} autoFocus onChange={e=>setNf({...nf,name:e.target.value})} placeholder="e.g. HTF Bias, Liquidity Target…"/></div>
            <div style={{marginBottom:16}}>
              <div style={{fontSize:8,letterSpacing:3,textTransform:'uppercase',color:'var(--text2)',marginBottom:8,fontFamily:'var(--mono)',fontWeight:600}}>Answer Type</div>
              <div className="type-tabs">{['dropdown','multiselect','text','checkbox'].map(t=>(<button key={t} className={`type-tab ${nf.type===t?'on':''}`} onClick={()=>setNf({...nf,type:t})}>{tl[t]}</button>))}</div>
            </div>
        ...
        <div style={{background:'var(--bg2)',border:'1px solid var(--line)',borderRadius:10,padding:'20px 24px'}}>
          ...
          <div style={{fontSize:14,color:'var(--text)',marginBottom:4,fontWeight:700}}>@{profile.username}</div>
          <div style={{fontSize:9,color:'var(--text3)',fontFamily:'var(--mono)'}}>Member since {profile.createdAt?new Date(profile.createdAt).toLocaleDateString('en-GB',{month:'long',year:'numeric'}):'—'}</div>
        </div>
      </div>
    </div>
  );
